 ...
 **enable_memory_monitoring**\은 서버의 힙 메모리 사용량 모니터링 여부를 지정한다. 값을 YES로 설정하면, 서버의 메모리 모니터링 기능을 활성화하여 서버의 힙 메모리 사용량을 지속적으로 추적 관리한다. 힙 메모리 사용량은 큐브리드 소스 코드에서 메모리 동적 할당이 발생하는 파일과 라인을 기준으로 추적되며, 동일한 위치에서 여러 번 메모리 할당이 발생하는 경우 메모리 사용량은 누적된다. 추적 중인 메모리가 해제되면 누적된 총 메모리 사용량에서 해제되는 메모리 크기 만큼 감산하여 실시간 힙 메모리 사용량을 계속 추적한다. 추적 중인 힙 메모리 사용량은 :ref:`memmon` 유틸리티를 통해 확인할 수 있다. 기본값은 NO이다.

.. note::

    *   윈도우 환경에서는 지원하지 않는다.
    *   메모리 사용량 추적의 자동화 과정에서 glibc와의 충돌로 인해 이를 막기 위해 glibc 내부에서 발생하는 메모리 사용량(STL 컨테이너)은 추적하지 않으며, 동일한 이유로 헤더 파일에서 발생하는 메모리 사용량은 추적하지 않는다.
    *   HA 환경 구성 시 슬레이브 노드의 경우 마스터 노드의 로그를 처리하는 과정에서 메모리 할당의 비중이 높아 이에 따른 메모리 사용량 모니터링의 비용이 커 성능의 문제로 이어질 수 있기 때문에 **enable_memory_monitoring**\의 지원 대상에서 제외하였다.
    *   **enable_memory_monitoring**\은 pmap -d, htop에서 표기되는 메모리 사용량과는 다를 수 있으며, 이는 큐브리드의 메모리 사용량 모니터링이 헤더 파일 및 glibc 내부에서 발생하는 메모리 할당에 대해 추적하지 않아 발생하는 차이 외에 다음과 같은 차이가 존재한다.
        *   pmap -d는 "writeable/private"항목에서 프로세스의 힙 메모리 사용량을 조회할 수 있으나 이는 운영체제의 메모리 관리 정책에 따른 프로세스의 메모리 점유량을 보여주는 것이기 때문에 차이가 발생할 수 있다.
        *   htop은 RES(Resident Set Size) 항목을 통해 프로세스의 메모리 사용량을 제공하나 이는 프로세스의 물리 메모리 사용량을 보여주는 것이기 때문에 프로세스의 힙 메모리 사용량 만을 확인할 수는 없다.
