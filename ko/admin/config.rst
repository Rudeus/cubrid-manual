...
.. note::
    ...
    *   HA 환경 구성 시 슬레이브 노드는 마스터 노드의 로그를 처리하는 과정에서 메모리 할당 비중이 높다. 이로 인해 메모리 사용량 모니터링의 비용이 증가하여 성능 문제가 발생할 수 있다. 따라서 슬레이브 노드에서는 **enable_memory_monitoring**\의 사용을 권장하지 않는다.

.. note::

    *   큐브리드에서 추적하고 있는 메모리 사용량은 pmap -d, htop에서 표기되는 메모리 사용량과는 다를 수 있다. 이는 큐브리드의 메모리 사용량 모니터링 기능이 헤더 파일 및 glibc 내부에서 발생하는 메모리 할당을 추적하지 않기 때문이며, 다음과 같은 추가적인 차이점이 존재한다.
