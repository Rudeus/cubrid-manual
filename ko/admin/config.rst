...
.. note::
    ...
    *   HA 환경 구성 시 슬레이브 노드의 경우 마스터 노드의 로그를 처리하는 과정에서 메모리 할당의 비중이 높아 이에 따른 메모리 사용량 모니터링의 비용이 커 성능의 문제로 이어질 수 있기 때문에 슬레이브 노드에서 **enable_memory_monitoring**\의 사용을 권장하지 않는다.
