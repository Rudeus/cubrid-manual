...
.. note::
    ...
    *   메모리 사용량 모니터링 기능을 사용하면 추적을 위한 연산으로 인해 추가적인 비용이 존재한다. 특히 HA 환경 구성 시 슬레이브 노드는 마스터 노드의 로그를 처리하는 과정에서 메모리 할당 비중이 높아 성능 문제가 발생할 수 있다. 따라서 슬레이브 노드에서는 사용을 권장하지 않는다.
